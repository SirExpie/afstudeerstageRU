...
for file in $(ls -f asm/*_contigs.fasta) ;
do
  echo $file
  $binmate/qc_asm_stats.py -i $file --report --datatable -o asm/
done

# Filter on contig size
echo "Filter on contig size "$min_ctg_len"..."
for file in $(ls -f asm/*_contigs.fasta) ;
do
  $binmate/qc_asm_stats.py -i $file --report --datatable --length $min_ctg_len --fasta -o asm/
done
echo "Filtering done..."

# READ MAPPING
# ------------------------------------------------------------------------------
echo $(ls -f asm/*_contigs*_gt$min_ctg_len.fasta)
for asm_file in $(ls -f asm/*_contigs*_gt$min_ctg_len.fasta) ;
do
# Map PE + SE reads to metagenome - BWA
  $binmate/map_bwa.sh $asm_file trim/ map/ $threads
...
# BinSanity
echo "Binsanity running..."
for asm in $(ls -f asm/*_contigs*_gt$min_ctg_len.fasta) ;
do
#asm=asm/*R1*_contigs*_gt$min_ctg_len.fasta
  if [ $(grep -c ">" $asm) -lt 75000 ] ; then
    cd bin/binsanity/
    $binmate/bin_binsanity.sh ../../$asm ../../map/pe/ $threads
    cd $proj_dir
  else
    echo "Deleting binsanity folders..."
...
echo "Generating CheckM quality report..."

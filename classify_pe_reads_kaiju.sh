...
# Usage
# classify_pe_reads_kaiju.sh <prefix> <kaiju_db> <trim_R1.fq.gz> <trim_R2.fq.gz> <out_dir> <threads>
...
# Classify reads
if [[ $r1 =~ \.gz$ ]]; then 
    kaiju -a greedy -e 5 -x -v -z $threads -t $kaiju_db/nodes.dmp -f $kaiju_db/kaiju_db_nr_euk.fmi -i <(gunzip -c $r1) -j <(gunzip -c $r2) -o $out_dir/$prefix.kaiju.out
else
  kaiju -a greedy -e 5 -x -v -z $threads -t $kaiju_db/nodes.dmp -f $kaiju_db/kaiju_db_nr_euk.fmi -i $r1 -j $r2 -o $out_dir/$prefix.kaiju.out
fi
